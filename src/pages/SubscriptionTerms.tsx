...
import { subscriptionTerms, subscriptionTiers } from '../data/studio';
import type { Navigate } from '../lib/router';

export default function SubscriptionTerms({ onNavigate }: { onNavigate: Navigate }) {
  return (
    <main>
      <PageHeader
        marker="01"
        markerLabel="Subscription terms"
        title={<>How the subscription actually works.</>}
        body="The operational terms behind the tiers — what a day means, what happens to unused days, what happens if a month runs long, and how to leave. Written to be read before you subscribe, not after."
        actions="none"
      />
      ...
      <section className="relative py-12 lg:py-16 border-b border-ink-700/40">
        <div className="mx-auto max-w-edge px-5 lg:px-8">
          <Reveal>
            <SectionMarker n="02" label="The caps" />
          </Reveal>
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-px bg-ink-700/40 border border-ink-700/40 rounded-2xl overflow-hidden">
            {subscriptionTiers.map((t, i) => (
              <Reveal key={t.id} delay={((i % 3) + 1) as 1 | 2 | 3} className="bg-ink-900 p-7">
                <div className="flex items-baseline justify-between gap-3">
                  <h2 className="font-display text-xl font-medium text-bone-50 tracking-tighter2">
                    {t.name}
                  </h2>
        ...
        <div className="mx-auto max-w-edge px-5 lg:px-8">
              ...
              <Reveal key={sec.n} delay={((i % 3) + 1) as 1 | 2 | 3}>
                <article className="grid grid-cols-12 gap-4 lg:gap-8 py-9 lg:py-10 bg-ink-950 -mx-3 px-3 lg:-mx-5 lg:px-5 rounded-lg">
                  <div className="col-span-12 lg:col-span-3">
                    ...
                    <h2 className="mt-3 font-display text-lg lg:text-xl font-medium text-bone-50 tracking-tighter2 leading-tight text-pretty">
                      {sec.title}
                    </h2>
                  </div>
          ...
          <Reveal delay={1} className="mt-10">
            <p className="text-sm text-bone-500 leading-relaxed text-pretty max-w-2xl">
              These are the operational terms of the engagement — how the work runs, month to
              month. They are not a substitute for a contract; where a client needs one, we sign
              theirs or provide ours.
            </p>
          </Reveal>
        </div>
      </section>
